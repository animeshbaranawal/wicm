...
public class UByteREACHDMutationFileReader extends WICMMutationFileReader<IntWritable, UByteBooleanIntervalData, UByteBooleanIntervalData> {
    ...
    @Override
    UByteBooleanIntervalData setVertexValue(String[] line) {
    ...
    @Override
    List<Edge<IntWritable, UByteBooleanIntervalData>> setEdges(String[] line) {
        if(getMode() == MODE.DELETE_VERTEX)
            return null;
        ...
        for (int n = startIndex; n < line.length; n=n+3) {
            edges.add(EdgeFactory.create(
                    ...
                    new UByteBooleanIntervalData(new UByteInterval(Integer.parseInt(line[n+1]), Integer.parseInt(line[n+2])))
            ));
        }
        return edges;
    }
