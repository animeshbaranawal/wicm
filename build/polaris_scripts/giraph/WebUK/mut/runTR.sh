...
org.apache.giraph.GiraphRunner in.dreamlab.wicm.algorithms.wicm.UByteREACH_D \
--yarnjars WICM-1.0-SNAPSHOT-jar-with-dependencies.jar \
--yarnheap 200 \
-vif in.dreamlab.wicm.io.mutations.formats.UByteREACHDTextInputFormat -vip $inputGraph \
-vof in.dreamlab.wicm.io.formats.UByteBooleanIdWithValueTextOutputFormat -op $outputDir"_mut" -w 1 \
...
-ca giraph.vertexResolverClass=in.dreamlab.wicm.graph.mutations.resolver.UByteREACHDVertexResolver \
-ca wicm.mutationReaderClass=in.dreamlab.wicm.io.mutations.UByteREACHDMutationFileReader \
...
##### dump logs
#appID=$(yarn app -list -appStates FINISHED,KILLED | grep "REACH" | sort -k1 -n | tail -n 1 | awk '{print $1}')
#echo $appID
#yarn logs -applicationId $appID > "TR_"$outputDir"_"$source"_mut.log"
